...
##' \code{\link{mongo.bson.from.buffer}()} may be used to turn the buffer into
##' a mongo.bson object.
##' 
...
##' 
##' These mongo.code values may also be present in a list and\cr will be
##' handled properly by \code{\link{mongo.bson.buffer.append.list}()} and\cr
...
##' \code{\link{mongo.find}()} and used to iterate over the records matching
##' the query.
##' 
...
##' externally managed pointer to the actual cursor data. This pointer is
##' stored in the "mongo.cursor" attribute of the object.
##' 
##' 
##' @name mongo.cursor
##' @docType class
...
##' 
##' Objects of class "mongo.gridfs" are used to store and/or access a "Grid
##' File System" (GridFS) on a MongoDB server.  While primarily intended to
##' store large documents that won't fit on the server as a single BSON object,
...
##' externally managed pointer to the actual data used to manage operations on
##' the GridFS.\cr This pointer is stored in the "mongo.gridfs" attribute of
##' the object.  The object also has a "mongo" attribute holding a pointer to
##' the mongo connection object used in creation to prevent garbage collection
##' on the mongo object while the mongo.gridfile is still active.
##' 
...
##' 
##' Objects of class "mongo.regex" represent regular expressions and are
##' strings with the options value stored in the "options" attribute.
##' 
##' See
...
##' their class so that \code{\link{mongo.bson.buffer.append}()} may detect
##' them and append the appropriate BSON code-typed value to a buffer.
##' 
...
##' 
##' Objects of class "mongo.undefined" are used to represent undefined values
##' in BSON documents.
##' 
##' mongo.undefined objects are strings (a character vector) with a single
##' value of "UNDEFINED"
##' 
