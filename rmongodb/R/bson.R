...
##' 
##' Get the number of bytes taken up by the BSON data attached to the
##' mongo.bson object
##' 
##' 
...
##' 
##' Releases the resources associated with a \link{mongo.bson} object. It is
##' not absolutely necessary to call this function since R's garbage collection
##' will eventually get around to doing it for you.
##' 
##' 
...
##' arrays).
##' 
##' This version is an alias of mongo.bson.print() so that print() will
##' properly handle the mongo.bson class.
##' 
##' 
...
##' 
##' This function is somewhat schizophrenic depending on the types of the
##' fields in the mongo.bson object. If all fields in an object (or
##' subobject/array) can be converted to the same atomic R type (for example
##' they are all strings or all integer, you'll actually get out a vector of
##' the atomic type with the names attribute set.
##' 
...
##' 
##' Note that \code{mongo.bson.to.list()} operates recursively on subobjects
##' and arrays and you'll get lists whose members are lists or vectors
##' themselves.  See \code{\link{mongo.bson.value}()} for more information on
...
##' object.  This bypasses the creation of a \link{mongo.bson.buffer},
##' appending fields one by one, and then turning the buffer into a mongo.bson
##' object with \code{\link{mongo.bson.from.buffer}()}.
##' 
##' Note that this function and \code{\link{mongo.bson.to.list}()} do not
##' always perform inverse conversions since mongo.bson.to.list() will convert
...
##' 
##' 
##' @param b (\link{mongo.bson}) The mongo.bson object through which to
##' iterate.
##' 
...
##' subobject or array.
##' @return (\link{mongo.bson.iterator}) An iterator initialized to 'before'
##' the start of the given mongo.bson object.
...
##' @return (\link{mongo.bson.iterator}) An iterator pointing to the field
...
##' integer \cr \code{\link{mongo.bson.timestamp}} \tab A
##' \link{mongo.timestamp} object \cr \code{\link{mongo.bson.long}} \tab A
##' double \cr }
##' 
##' Special handling:
##' 
...
##' appropriately attributed R object is returned; otherwise, a list is
##' returned containing the subfields.
##' 
##' \code{\link{mongo.bson.array}}: If all fields of the array are of the same
##' atomic type, a vector of that type is returned.  (Multidimensinal arrays
##' are detected and the \code{dims} attribute will be set accordingly.  Arrays
##' of complex values are also detected as above). Otherwise, a list is
##' returned containing the subfields.
##' 
##' \code{\link{mongo.bson.binary}}: If non-zero, the subtype of the binary
...
##' reference).
##' 
##' Note that this BSON data type is deprecated and rmongodb provides no
##' support for it. Attempting to fetch the value of a dbref with
...
##' performs the inverse operation of \code{\link{mongo.oid.from.string}()}.
##' 
##' This function is an alias of \code{\link{mongo.oid.to.string}()} so that
##' the class mechanism of R allows it to be called simply by
...
##' \code{\link{mongo.bson.buffer.append}()} or for embedding in a list such
##' that\cr \code{\link{mongo.bson.buffer.append.list}()} will properly insert
##' a code value into the mongo.bson.buffer object.
##' 
##' 
...
##' @param options (string) Options governing the parsing done with the
##' pattern.
...
##' \code{dimnames} attribute is ignored and a nested array is appended.\cr
##' (Use \code{\link{mongo.bson.buffer.append.object}()} if you want to
##' preserve \code{dimnames}).
##' 
...
##' 
##' In the last case, a single as.boolean is appended as the value of the
##' field.
##' @return TRUE if successful; otherwise, FALSE if an error occured appending
##' the data.
##' @seealso \link{mongo.bson},\cr \link{mongo.bson.buffer},\cr
...
##' \code{dimnames} attribute is ignored and a nested array is appended.\cr
##' (Use \code{\link{mongo.bson.buffer.append.object}()} if you want to
##' preserve \code{dimnames}; however, this can't append value as longs).
##' 
##' If value has a names attribute, a subobject is appended and the subfields
##' are given the indicated names.
##' 
...
##' 32-bit number. At such time, many applications, including R and this
##' driver, will need to address that issue.
##' 
##' 
##' @param buf (\link{mongo.bson.buffer}) The buffer object to which to append.
##' @param name (string) The name (key) of the field appended to the buffer.
...
##' This function appends such an indicator as the type of a field with its
##' value.
##' 
##' 
##' @param buf (\link{mongo.bson.buffer}) The buffer object to which to append.
...
##' 
##' BSON has a special field type to indicate binary data. This function
##' appends such an indicator as the type of a field with its value.
##' 
##' If value has a \code{dims} attribute of length > 1, any \code{names} or
##' \code{dimnames} attribute is ignored and a nested array is appended.\cr
...
##' mongo.binary.binary is used.  The following constants are defined:
##' \itemize{ \item\code{\link{mongo.binary.binary}} (0L)
...
##' detect the type of the \code{value} parameter and perform the same action
##' as the specific functions.  These functions are: \itemize{ \item
...
##' 
##' \code{\link{mongo.bson.buffer.append.long}()} is missing from the above
##' list since R has no 64-bit long integer type.  If you wish a value to be
##' stored in the BSON data as a long you must explicity call that function.
##' 
##' All of the above functions will lose the attributes of the object other
##' than "names". When vectors of length > 1 are appended, "names" are
##' preserved.\cr \code{\link{mongo.bson.buffer.append.object}()} gets around
##' this shortcoming and allows most R objects to be stored in a database
...
##' 
##' Note that the value parameter must be a true list, not an vector of a
##' single atomic type.
##' 
...
##' frames, matrices and arrays for instance; although, empty objects, such as
##' a data frame with no rows, are not permitted.
##' 
...
##' parameter.\cr \code{xxx} will be substituted with the low level value of
##' the object (as would be appended by
...
##' 
##' \code{\link{mongo.bson.buffer.finish.object}()} must be called when
##' finsihed appending subfields.\cr (\code{mongo.bson.buffer.start.object()},
##' \code{mongo.bson.buffer.start.array()})\cr and
##' \code{mongo.bson.buffer.finish.object()} may be called in a stackwise
##' (LIFO) order to further nest documents and arrays.
##' 
##' 
##' @param buf (\link{mongo.bson.buffer}) The buffer object to which to append.
...
##' \code{\link{mongo.bson.buffer.finish.object}()} must be called when
##' finished appending the elements of the array.
##' 
##' (\code{mongo.bson.buffer.start.object()},
##' \code{mongo.bson.buffer.start.array()}) and\cr
##' \code{mongo.bson.buffer.finsih.object()} may be called in a stackwise
##' (LIFO) order to further nest arrays and documents.
##' 
...
##' \code{mongo.bson.buffer.finish.object()} may be called in a stackwise
##' (LIFO) order to further nest documents.
##' 
##' This function must also be called to finish arrays.
##' 
##' 
##' @param buf (\link{mongo.bson.buffer}) The buffer object on which to finish
##' a subobject.
##' @return TRUE if successful; otherwise, FALSE if an error occured appending
##' the data.
##' @seealso \link{mongo.bson},\cr \link{mongo.bson.buffer},\cr
