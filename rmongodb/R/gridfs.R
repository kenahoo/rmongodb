...
##' 
##' Create a \link{mongo.gridfs} object used to access and store "grid files"
...
##' @return (\link{mongo.gridfs}) An object to be used for subsequent
##' operations on the grid file store.
...
##' GridFS.\cr If remotename=="" (the default), the remote file will be known
##' by the given \code{filename}.
...
##' stores the entire piece of data file on the server, breaking it up into
##' 256K chunks as necessary.
##' 
...
##' 
##' Create a \link{mongo.gridfile.writer} object used to buffer many writes to
##' a single GridFS file. Once the mongo.gridfile.writer is created, use
...
##' \code{query} may be a list which will be converted to a mongo.bson object
...
##' 
##' Get the chunk count of a \link{mongo.gridfile}. This is the number of
##' chunks into which the gridfile is broken up on the server.
##' 
##' 
##' @param gridfile A (\link{mongo.gridfile}) object.
...
##' 
##' @param gridfile A (\link{mongo.gridfile}) object.
##' @return (string) The content.type (remote name) of \code{gridfile}. This
##' may be an empty string if no content type is associated with the gridfile.
##' @seealso \code{\link{mongo.gridfs}},\cr \code{\link{mongo.gridfs.find}},\cr
##' \link{mongo.gridfile},\cr \code{\link{mongo.gridfile.get.descriptor}},\cr
##' \code{\link{mongo.gridfile.get.filename}},\cr
##' \code{\link{mongo.gridfile.get.length}},\cr
...
##' 
##' Read raw data from a \link{mongo.gridfile}.  The data read may span
##' multiple chunks.
##' 
##' A mongo.gridfile file maintains a current read position which is advanced
##' by the size of each read.  This position is initially at offset 0.
##' 
...
##' @return (raw) The data read from emphgridfile. The length of this vector
##' may be less than the requested size if there was not enough data remaining
##' to be read. This length could also be 0 if an error occured during the
##' operation. Check \code{\link{mongo.get.err}()} of the associated mongo
...
##' file to a connection. If the connection is open, it must be in binary
##' output mode; otherwise, the connection is opened in binary output mode and
##' closed afterwards.
##' 
##' 
##' @param gridfile A (\link{mongo.gridfile}) object.
